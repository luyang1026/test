'use strict';
(function(){
	angular.module('ly.commonAPI',[])
		.factory('commonAPI',function($http){
			return{
				getProvinces:function(){
					var res = [];
					for (var i = 0; i < localData.province.length; i++) {
						res.push(localData.province[i].name);
					}
					return res;
				},
				getCities:function(provinceName){
					for (var i = 0; i < localData.province.length; i++) {
						if(localData.province[i].name == provinceName){
							return localData.province[i].cities;
						}
					}
				}
			}
	...
	var localData = {
		province : [
		  ...
		  { name: "河北", cities: ["石家庄", "秦皇岛", "廊坊", "保定", "邯郸", "唐山", "邢台", "张家口", "承德", "沧州", "衡水"] },
